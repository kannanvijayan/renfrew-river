...
export type ShowConnectionFailedErrorCallbackApi = {
  switchToUnconnectedMainMenuView(): void;
}

export default class ShowConnectionFailedErrorView extends PIXI.Container {
  private readonly graphics: PIXI.Graphics;

  private readonly menuTitle: GameMenuTitle;

  constructor(opts: {
    callbackApi: ShowConnectionFailedErrorCallbackApi,
  }) {
    ...
    const errorMsg = new ErrorMessage({ text: "Connection Failed" });
...
class OkButton extends MenuButton {
  constructor(opts: { callbackApi: ShowConnectionFailedErrorCallbackApi }) {
    const { callbackApi } = opts;
    super({
      name: "ShowConnectionFailedError.Ok",
      text: "Ok",
      width: 100,
      onClickListener: () => {
