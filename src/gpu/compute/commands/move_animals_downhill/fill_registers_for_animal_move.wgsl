...
@group(0) @binding(3)
var<storage, write> out_register_files: array<ShadyRegisterFile>;

@group(0) @binding(4)
var<storage, write> out_start_pcs: array<u32>;
...
fn fill_registers(
  ...
  if (packed_cell_coord_is_invalid(animal_data.position)) {
    out_start_pcs[animal_id] = 0xFFFFFFFFu;
    return;
  }
  out_start_pcs[animal_id] = uniforms.start_pc;

  let my_cell = packed_cell_coord_to_cell_coord(animal_data.position);
  let my_elev = read_cell_elevation_unchecked(my_cell.col_row);
  out_register_files[animal_id].regs[15u] = i32(my_elev);

  for (var dir = MIN_HEX_DIR; dir <= MAX_HEX_DIR; dir++) {
    let adj_cell = hexcell_adjacent_checked(
      uniforms.world_dims, my_cell.col_row, dir);
    var write_elev: i32 = -1;
    if (!hexcell_is_invalid(adj_cell)) {
      write_elev = i32(read_cell_elevation_checked(adj_cell));
    }
    out_register_files[animal_id].regs[16u + dir] = write_elev;
  }
}
