...
fn calc_map_histo_branch(
  ...
  let output_entry_idx: u32 = output_idx * num_buckets;
  ...
  for (var j = 0u; j < area_dims.y; j++) {
    for (var i = 0u; i < area_dims.x; i++) {
      ...
      let input_entry_index = input_index * num_buckets;
      for (var n = 0u; n < num_buckets; n++) {
        let entry_count = input_buffer[input_entry_index + n];
        output_buffer[output_entry_idx + n] += entry_count;
      }
    }
  }
}
