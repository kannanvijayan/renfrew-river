...
struct Uniforms {
  world_dims: vec2<u32>,
  src_value_range: vec2<i32>,
  dst_value_range: vec2<i32>,
  entry_size: u32,
  selector: u32
};
...
fn rescale_map_data(
  ...
  let src_value_range = uniforms.src_value_range;
  let dst_value_range = uniforms.dst_value_range;
  ...
  let input_value: i32 = i32(
    (input_word >> sel_fmt.shift) &
    format_selector_get_mask(sel_fmt)
  );

  output_buffer[cell_idx] = u32(
    rescale_value(input_value, src_value_range, dst_value_range)
  );
}

fn rescale_value(x: i32, from_range: vec2<i32>, to_range: vec2<i32>) -> i32 {
  let from_size: i32 = from_range.y - from_range.x;
  let to_size: i32 = to_range.y - to_range.x;

  // Scale the value to the new range.
  let scaled_value = ((x - from_range.x) * to_size) / from_size + to_range.x;
  return clamp(scaled_value, to_range.x, to_range.y);
}
