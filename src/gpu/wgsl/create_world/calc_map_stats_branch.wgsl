...
@group(0) @binding(1)
var<storage, read> input_buffer: array<Statistics>;

@group(0) @binding(2)
var<storage, read_write> output_buffer: array<Statistics>;

@compute
@workgroup_size(8, 8)
fn calc_map_stats_branch(
  ...
  var stats: Statistics = statistics_new();

  let tl_xy = grid_xy * area_dims;
  for (var j = 0u; j < area_dims.y; j++) {
    for (var i = 0u; i < area_dims.x; i++) {
      let input_xy = tl_xy + vec2<u32>(i, j);
      if (input_xy.x >= world_dims.x || input_xy.y >= world_dims.y) {
        continue;
      }
      let input_index = hexcell_index(world_dims, input_xy);

      let dbg_old_min = stats.min;
      stats = statistics_merge(stats, input_buffer[input_index]);
    }
  }

  let output_idx: u32 = hexcell_index(grid_dims, grid_xy);
  output_buffer[output_idx] = stats;
}
